...
public class Services {
	...
	Services(Connection con, Statement st) {
		try {
			st.executeUpdate("update employee set salary=salary+(salary/10)");
		} catch (Exception e) {
		}
	}
	...
	public void display(Connection con, Statement st) {
		try {
			...
			while (rs.next()) {
				System.out.println(
						"-----------------------------------------------------------------------------------------");
				System.out.println("    EmpId : " + rs.getInt(1) + " | Name : " + rs.getString(2) + " | Age : "
						+ rs.getInt("age") + " | Designation : " + rs.getString("designation") + " | Salary :"
						+ rs.getInt("salary"));
				System.out.println(
						"-----------------------------------------------------------------------------------------");
			}
	...
	public void displayByDesignation(Connection con, Statement st, int desig) {
		...
		if (desig == 3)
			job = "Clerk";
		try {
			ResultSet rs = st.executeQuery("select * from employee where job = '" + job + "'");
			while (rs.next()) {
				System.out.println(
						"-----------------------------------------------------------------------------------------");
				System.out.println("    EmpId : " + rs.getInt(1) + " | Name : " + rs.getString(2) + " | Age : "
	...
	public void raiseByID(int id, Statement st, Connection con) {
		try {
			st.executeUpdate("update employee set sal=sal+(sal/10) where id=" + id);
		} catch (Exception e) {
		}
